...
这是机械设计自动化最核心的步骤之一，涉及从结构化参数或自然语言中生成 可执行的 CAD 参数化建模脚本，并可通过 FreeCAD / CadQuery / OCC 环境进行验证。

下面直接给出 Step 2 的完整 README 部分（Markdown 完整版）。

Step 2：单零件参数化建模（Parametric Part Modeling）

本步骤关注如何从结构化需求（Step 1 输出）或自然语言描述中，生成 参数化 CAD 脚本，并在实际 CAD 环境中执行验证。
...
可执行的 参数化建模代码（如 CadQuery、FreeCAD Python、OpenSCAD、Fusion360 FeatureScript）
